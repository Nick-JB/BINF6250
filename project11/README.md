...
# Struggles
Prior to any scripting, one of the biggest challenges we faced was with the foundational pseudocode.  Mapping alignment columns to match, insert, and delete states, handling gaps appropriately, and constructing emission distributions required careful attention to detail. Understanding the algorithm and translating our ideas from concepts to pseudocode took us a lot longer than we had planned, but it also helped make the script writing much faster later on once we got to that step. Reading through HMM.py also proved to be quite time-consuming, and we had to go back several times to make sure our profile HMM didn't cause any errors with the logic in the baseHMM. One of the first challenges we faced with this logic stemmed from a mismatch between the design of BaseHMM and the requirements of a Profile HMM. BaseHMM represents hidden states as a single concatenated string and iterates over it character by character. While this works for single-character state names, it breaks for multi-character states such as M1 or D3, since a string is interpreted as individual characters rather than complete state labels. Once we got past the initial hurdles with our pseudocode and logic, the rest was not as complicated. However, with the time constraints and scheduling conflicts, we had to allocate most of our collaboration to be asynchronous. Additionally, the scores that we are getting from the forward algorithm applied on our model don't seem to differentiate well from between protein motif positive sequence and negative sequences. It is unclear whether this is an issue with our algorithm or a limitation of the small dataset creating weaker or less accurate probability scores, but either way given more time, this would need further exploration to ensure that our model is able to differentiate between positive and negative with reasonable accuracy.

# Personal Reflections
## Nicholas Bottomley
This felt like one of the more confusing assignments to implement even though most of the algorithms had already been completed. I think I share the feeling with my group that it was a little difficult at times to work with the base hmm object that we were provided with. Especially when it tries to store the hidden states as a string instead of a list or other iterable. It is understandable how a string could be a attractive for storing the states, but I would've expected some way to unpack it instead of just assuming that all states would have a single character name, leading to issues with ML, IL, DL, naming conventions for the states. Another hurdle I ran into with the implementation of the profile HMM was understanding the sum of all the parts. Each step seemed simple enough, but figuring out how to connect them was very difficult for me. Finally, I wasn't able to figure out how to implement the baum-welch training on our model since the states are ordered based on the data provided, but the EM of the Baum-Welch function may not respect the naming or ordering of the states, leading to a technically correct set of probabilities, but maybe not one that is still properly ordered. With all of the things coming to a head at this time of year, it was also difficult to find time when everyone in our group was available to meet, so we mostly messaged with progress updates instead. Either way, I am happy with what we were able to implement and feel like I now understand profile HMMs much better than I did prior to this week.
