...
# Struggles
One of the primary challenges we faced was understanding the underlying concepts before implementation. We found that we needed to spend significant time working through the theory to fully grasp how the Forward and Backward algorithms contribute to expected counts. Rather than rushing into coding, we deliberately slowed down to ensure a solid conceptual foundation, which ultimately improved the correctness of our implementation but extended development time. Translating our concepts from formula to code also proved to be a confusing endeavor at first, but through our debugging, we were able to make sense of what needed to happen and how. Once we got over the conceptual hurdle, the code and testing were not as difficult, but still took some time to wrap our heads around.

# Personal Reflections
## Group Leader
Out of the algorithms that we have had to complete thus far, I would definitely say this has been the hardest. In many cases it was just a matter of understanding what the algorithm was doing at which point it became pretty straightforward to implement. For Baum-Welch however, it felt as though understanding the algorithm led me straight into another brick wall of figuring out how to implement it. I knew that I had the tools necessary to do it from the previous week, but it was still difficult to wrap my head around all of the updating values and making sure that probabilities were in log space when expected to be and not in log space when we didn't want them to be. Overall, it was very rewarding to see a valid output from our code.
